...
GEN_MODEL = llm.CLAUDE_MODEL     # 생성·채점 같은 모델 (llm.py)
...
LABELS = [("원고:blog", "c98a00", "블로그 원고 PR — merge 가 승인, close 가 반려"),
          ...
          # 카드뉴스 PR 은 card-news 레포의 render.yml 이 GH_PAT 으로 연다(2026-09-21, 증분 5).
          # 그쪽도 `gh label create --force` 를 부르지만, 라벨의 뜻은 여기 한 곳에 적는다.
          ("원고:card", "c98a00", "카드뉴스 덱 PR — merge 하면 발행 대기, 목 09:00 에 인스타로 나간다"),
          ("원고:newsletter", "c98a00", "뉴스레터 원고 PR — merge 하면 발행 대기, 금 09:00 에 나간다"),
